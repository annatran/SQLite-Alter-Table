...
//Only one result set should be open at any time
public class SQLite_RenameColumn {
	...
	public void executeRename(String Table, String OldColumn, String Column) throws Exception {
		...
		try {
				...
				if(checkIfPrimaryKey("tempTable",ColumnNames.get(0))){
					if(ColumnNames.get(0).equals(prevColName)) {
						System.out.println("First column is the Primary Key. Cannot rename PK.");
					}
				...
				for (int i = 1; i < count; i++)
				{
				    ...
				    {
				    	if(checkIfPrimaryKey("tempTable",prevColName)) {
				    		System.out.println("Cannot rename Primary Key.");
				    		sql = "ALTER TABLE "+Table+" ADD COLUMN "+Column+" "+ColumnTypes.get(i)+" PRIMARY KEY;";
				    		//statement.executeUpdate(sql);
				    	}
				    	else {
				    		sql = "ALTER TABLE "+Table+" ADD COLUMN "+Column+" "+ColumnTypes.get(i)+";";
				    		statement.executeUpdate(sql);
				    	}
				    }
				    else
